...
async def custom_lifespan(app: FastAPI):
    ...
    #Flipped Assignment 4 updates (Part 3.11: change the loading of the classifier)
    # Setup the .clr property to be a distionary rather that just being the Turi model
    #app.clf = [] # start app with no classifier
    # Define .clf as a dictionary with key:dsid and obect:classifier
    app.clf = {}
...
async def train_model_turi(dsid: int):
    ...
    #app.clf = model 
    #Flipped 4 - updates 3.11 Update to use .clf dictionary to store models
    app.clf.update({dsid:model}) 
    #debug
    print("classifier dictionary:",app.clf)
...
async def predict_datapoint_turi(datapoint: FeatureDataPoint = Body(...)):
    ...
    #Flipped 4 - updates part 3.12: Change the loading of the classifier.
    # 
    #if(app.clf == []):
    # If dsid is not in the dictionary, try loading that model from the file in the model folder
    if (datapoint.dsid not in app.clf): 
        print("Datapoint not in current dictionary. Trying to LoadTuri Model From file")
        model_path = f"../models/turi_model_dsid{datapoint.dsid}"

        if os.path.exists(model_path):  # Check if the file exists
            try:
                app.clf[datapoint.dsid] = tc.load_model(model_path)
                print(f"Model for dsid {datapoint.dsid} loaded successfully.")
            except Exception as e:
                print(f"Error loading model for dsid {datapoint.dsid}: {e}")
                return
        else:
                print(" No trained Model file found for dsid {datapoint.dsid}") 
                return
        
        #app.clf = tc.load_model("../models/turi_model_dsid%d"%(datapoint.dsid))

        #Update dictionary with model 
        # app.clf.update({datapoint.dsid:"../models/turi_model_dsid%d"%(datapoint.dsid)}) 
    ...
    curr_mdl = app.clf[datapoint.dsid]
    pred_label = curr_mdl.predict(data)
